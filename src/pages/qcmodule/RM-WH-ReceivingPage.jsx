...
import PrintChecklistModal from "./Print-Checklist-Modal";

const fetchWHReceivingApi = async (pageNumber, pageSize, search) => {
  const res = await apiClient.get(
    `Receiving/GetAllAvailableForWarehouseReceivingWithPaginationOrig?pageNumber=${pageNumber}&pageSize=${pageSize}&search=${search}`
  );
  return res.data;
};
...
const RMWHReceivingPage = ({ fetchNotification }) => {
  ...
  return (
    <Flex p={5} w="full" flexDirection="column">
            ...
            <Tbody>
              {whData.warehouse?.map((wh) => (
                <Tr key={wh.id}>
                  ...
                  <Td>{wh.supplier}</Td>
                  <Td>
                    {wh.quantityOrdered.toLocaleString(undefined, {
                      maximumFractionDigits: 2,
                      minimumFractionDigits: 2,
                    })}
                  </Td>
                  <Td>
                    {wh.actualGood.toLocaleString(undefined, {
                      maximumFractionDigits: 2,
                      minimumFractionDigits: 2,
                    })}
                  </Td>
                  <Td>{wh.reject}</Td>
                  <Td>
                    {wh.expirationDate ? wh.expirationDate : "Not Expirable"}
                  </Td>
                  <Td>{wh.qC_ReceivedDate}</Td>
                  <Td>
                    <Button
                      onClick={() => handlePrint(wh.id)}
                      colorScheme="blue"
      ...
      {isPrint && <PrintChecklistModal isOpen={isPrint} onClose={closePrint} receivingId={receivingId} />}
