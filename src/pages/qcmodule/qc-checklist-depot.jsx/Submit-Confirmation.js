...
const SubmitConfirmation = ({
  ...
  const submitDocumentationChecklist = [];
  for (const obj of documentationChecklist) {
    for (const key in obj) {
      if (obj[key] === true) {
        submitDocumentationChecklist.push(key);
      }
    }
  }

  const submitOtherConformanceDetails = [];
  for (const obj of otherConformanceDetails) {
    for (const key in obj) {
      if (obj[key] === true) {
        submitOtherConformanceDetails.push(key);
      }
    }
  }

  const submitHygieneDetails = [];
  for (const obj of hygieneDetails) {
    for (const key in obj) {
      if (obj[key] === true) {
        submitHygieneDetails.push(key);
      }
    }
  }

  const submitChecklistString = [
    {
  ...
  const submitData = {
    pO_Receiving: {
      ...
      monitoredBy: submitReceiving.monitoredBy,
    },
    checklistsString: submitChecklistString,
  };
