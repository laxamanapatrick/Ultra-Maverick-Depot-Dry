...
const ScannedModalRejection = ({
  ...
  const { setQuantity, setRemarks, setSumQuantity, setSubmitRejectData } =
  ...
  const addNewRowHandler = () => {
    ...
    setQuantity("");
    setRemarks("")
    remarksDisplay.current.value = '';
    remarksDisplay.current.selectedIndex = '';
  };

  const deleteRejectionHandler = (data) => {
    setDisplayData(displayData.filter((row) => row.remarks !== data));
  };
            ...
            <Flex justifyContent="space-between">
              <FormLabel w="40%">
                ...
                <Input
                  ...
                  borderColor={remarks && !quantity ? "danger" : "none"}
                />
                {remarks && !quantity && (
