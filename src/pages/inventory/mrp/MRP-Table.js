...
export const MRPTable = ({
  ...
  const {
    isOpen: isRestockingOpen,
    onClose: closeRestocking,
    onOpen: openRestocking,
  } = useDisclosure();
  const handleRestockingModal = () => {
    setPageSize(10000);
    setIsLoading(true);
    openRestocking();
  };
  ...
  return (
    <Flex w="full" justifyContent="center" flexDirection="column">
      ...
      <Flex mt={5} justifyContent="space-between" w="full">
        <Button colorScheme="red" onClick={handleRestockingModal}>
          Check All Critical Levels
        </Button>
        <Stack>
          <Pagination
        ...
        <ModalContent>
          <ModalHeader>
            <Flex justifyContent="center">
              Raw Materials suggested for re-stocking
            </Flex>
          </ModalHeader>
          <ModalCloseButton onClick={handleClose} />
            ...
            <PageScrollReusable minHeight="617px" maxHeight="618px">
              {isLoading ? (
                ...
                <Table size="sm" variant="simple">
                  ...
                  <Tbody>
                    {mrpData?.inventory?.map(
                      (item, i) =>
                        item.soh <= item.bufferLevel && (
                          <Tr key={i}>
                            {/* <Td>{i + 1}</Td> */}
                            <Td>{item.itemCode}</Td>
                            <Td>{item.itemDescription}</Td>
