...
export const ActualItemQuantity = ({
  ...
  const expirationDate = !barcodeData?.orders?.expirationDate
    ? null
    : moment(barcodeData?.orders?.expirationDate).format("yyyy-MM-DD");
  ...
  return (
    <Flex w="full" flexDirection="column">
      <Text color="white" bgColor="secondary" textAlign="center">
      ...
      <HStack justifyContent="space-between" mt={2}>
        ...
        <HStack spacing={5}>
          ...
          <Text bgColor="gray.200" border="1px" px={12} fontSize="sm">
...
const AvailableBarcodeModal = ({
  ...
  const selectId = (id, actualGood) => {
    if (id) {
      setWarehouseId(id);
      if (Number(actualGood) > Number(qtyOrdered)) {
        setQuantity(qtyOrdered);
      } else {
        setQuantity(actualGood);
      }
      onClose();
    }
  };
                  ...
                  {availableBarcode?.map((items) =>
                    items.actualGood <= 0 ? null : (
                      <Tr key={items.id}>
                        ...
                        <Td>
                          <Button
                            onClick={() => selectId(items.id, items.actualGood)}
                            size="xs"
                            colorScheme="blue"
