...
const fetchRawApi = async (pageNumber, pageSize, status, search) => {
  const res = await apiClient.get(
    `RawMaterial/GetAllRawMaterialWithPaginationOrig/${status}?pageNumber=${pageNumber}&pageSize=${pageSize}&search=${search}`
  );
  return res.data;
};
...
const RawMaterialsPage = () => {
  ...
  const changeStatusHandler = (id, status) => {
    ...
    apiClient
      .put(`RawMaterial/${routeLabel}/${id}`, { id: id })
      .then((res) => {
  ...
  return (
    ...
    <Flex p={5} w="full" flexDirection="column">
            ...
            <Tbody>
              {raws.rawmaterial?.sort((a, b) => b.id - a.id)?.map((raw) => (
                <Tr key={raw.id}>
                  ...
                  <Td>{raw.uom}</Td>
                  <Td>
                    {raw.bufferLevel.toLocaleString(undefined, {
                      maximumFractionDigits: 2,
                      minimumFractionDigits: 2,
                    })}
                  </Td>
                  <Td>{raw.isExpirable ? "Yes" : "No"}</Td>
                  <Td>{raw.dateAdded}</Td>
                  <Td>{raw.addedBy}</Td>
                  <Td>
                    <Button
                            ...
                            <PopoverContent>
                              ...
                              <PopoverBody>
                                <VStack>
                                  {raw.isActive === true ? (
                                    <Text>
                                      Are you sure you want to set this raw
                                      material inactive?
                                    </Text>
                                  ) : (
                                    <Text>
                                  ...
                                  <Button
                                    ...
                                    onClick={() =>
                                      changeStatusHandler(raw.id, raw.isActive)
                                    }
                                  >
                                    Yes
                                  </Button>
      ...
      <Flex justifyContent="space-between" mt={5}>
        ...
        {isPrintAll && (
          <PrintAllBarcodeModal isOpen={isPrintAll} onClose={closePrintAll} />
        )}
  ...
  const submitHandler = (data) => {
    try {
      if (data.formData.id === "") {
        ...
        setisLoading(true);
        const res = apiClient
          .post("RawMaterial/CreateNewRawMaterials", data.formData)
          .then((res) => {
        ...
        const res = apiClient
          .put(
            `RawMaterial/UpdateRawMaterials/${data.formData.id}`,
            data.formData
          )
          .then((res) => {
          ...
          <DrawerContent>
            ...
            <DrawerBody>
              <Stack spacing="7px">
                ...
                <Box>
                  <Stack>
                    <Text fontWeight="semibold">Expirable:</Text>

                    <Select
                      {...register("formData.isExpirable")}
                      placeholder="Specify is item is expirable"
                    >
                    ...
                    <Text color="danger" fontSize="xs">
                      {errors.formData?.isExpirable?.message}
                    </Text>
                  </Stack>
                </Box>
              </Stack>
